...
def M3_PTF(Texture):
    ...
    class FNN_test(nn.Module):
        ...
        def forward(self,x):
            ...
            out2[:,[0]]=-3+2.3010*out[:,[0]]  # log10()alpha
            out2[:,[1]]=0.0043+1.17176988527304*out[:,[1]] # log10()n
            out2[:,[2]]=-2+2.17609125905568*out[:,[2]] # log10()m
            out2[:,[3]]=-0.698970004336019+0.628388930050312*out[:,[3]] # log10()ths
            return out2
    FNN=FNN_test(input_size=4,hidden_size=374,output_size=4,layers=3)
    FNN.to(device=device)

    FNN.load_state_dict(torch.load("FNN_model/M3_H3_SWRC.pth"))
    ...
    FNN.load_state_dict(torch.load("FNN_model/M3_H3_K.pth"))
    ...
    # pd.concat([pd.DataFrame(out_para_SWRC),pd.DataFrame(out_para_K)],axis=1).to_csv('M3_Para.csv',header=None) # Save
    pd_Para=pd.concat([pd.DataFrame(out_para_SWRC),pd.DataFrame(out_para_K)],axis=1)
    pd_Para.columns=['alpha','n','m','ths','K(ha)','Ks']
